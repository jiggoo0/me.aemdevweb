...
/**
 * Hero Component - Specialist Identity Version 2026
 * -------------------------------------------------------------------------
 * นำเสนอตัวตน "อลงกรณ์ ยมเกิด" ผ่านประสบการณ์และผลงานจริงที่จับต้องได้
...
export default function Hero() {
  return (
    <section className="relative w-full overflow-hidden bg-white py-16 md:py-24 lg:py-32">
      {/* Background Decor: เน้นความสะอาดตาและมีมิติด้วยแสงและเงา */}
      ...
      <Container>
        <div className="flex flex-col items-center gap-20 lg:flex-row lg:gap-24">
          {/* ส่วนข้อมูลหลัก (Identity Content) */}
          <div className="flex-1 space-y-12 text-center md:text-left">
            ...
            <div className="space-y-6">
              <h1 className="font-heading text-6xl leading-[0.95] font-black tracking-tighter text-slate-950 md:text-8xl lg:text-9xl">
            ...
            <p className="font-body mx-auto max-w-2xl text-lg leading-relaxed font-bold text-slate-500 md:mx-0 md:text-xl md:leading-9">
              ผู้อยู่เบื้องหลังระบบเทคนิคของ{" "}
              ...
              และที่ปรึกษาด้านโครงสร้างเว็บไซต์ที่เน้นผลลัพธ์ผ่าน{" "}
              <span className="font-black text-blue-600 underline decoration-blue-600/30 decoration-4 underline-offset-8">
                SEO Organic
              </span>{" "}
              ระดับผู้เชี่ยวชาญ
            </p>

            {/* การเชื่อมโยงเครือข่ายสังคม (Social Hub) */}
            <div className="flex justify-center md:justify-start">
              <SocialLinks />
            </div>

            {/* ส่วนกระตุ้นการตัดสินใจ (Call to Action) */}
            <div className="flex flex-col justify-center gap-5 pt-4 sm:flex-row md:justify-start">
              ...
              >
                <Link href="/contact" className="flex items-center">
                  เริ่มปรึกษาโปรเจกต์ <ArrowRight className="ml-3 h-6 w-6" />
                </Link>
              </Button>
              <Button
                asChild
              ...
              >
                <Link href="/projects">ดูผลงานทั้งหมด</Link>
              </Button>
            </div>
          </div>

          {/* ส่วนภาพลักษณ์ (Visual Authority) */}
          <div className="relative aspect-square w-full max-w-[580px] flex-1">
            {/* เลเยอร์ตกแต่งเพิ่มมิติเชิงลึก */}
            ...
            {/* Floating Badge: ยืนยันความสำเร็จเชิงประจักษ์ */}
            <div className="absolute -bottom-8 -left-8 hidden rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-2xl transition-all duration-500 hover:-translate-y-3 sm:block">
              <div className="mb-3 flex items-center gap-2 text-[10px] font-black tracking-[0.3em] text-blue-600 uppercase">
                <ShieldCheck size={16} /> Verified Success
              </div>
              <p className="text-2xl leading-none font-black tracking-tight text-slate-950">
                Unlink-th Lead
              </p>
              <div className="mt-3 flex items-center gap-2 text-[10px] font-black tracking-widest text-slate-400 uppercase">
